      ...
      <h1 className='text-center text-3xl font-medium'>Loan Details</h1>
        ...
        <div className='middle_loanHist  h-[90%] rounded-lg overflow-scroll p-6'>
...

          {
            ...
            //user.loans.map()....
            user.pending_loans.map((item, index) => (
              // <p key={index}>{}</p>
              ...
              <div key={index} className="inner_loanHist font-medium flex  bg-white flex-col gap-2 m-4 p-4 rounded-lg text-xl">
                ...
                <p >Loan status: <span className={item.status === "active" ? `text-green-400` : `text-red-500`}>{item.status}</span></p>
              </div>
...
//             //user.loans.map()....
//             loans.filter((item) =>item.status == "pending").map((item, index) => (
//               // <p key={index}>{}</p>
